...
export namespace tr {
	 ...
	 ******************************************************************************************************************/
    template <class T> concept CustomEventType = std::constructible_from<T, CustomEventBase> && std::is_convertible_v<T, CustomEventBase>;
	...
	public:
        /**************************************************************************************************************
	     * Constructs an event from a custom event type.
         *
         * @tparam T A custom event type.
         *
         * @exception std::bad_alloc If allocating generic values failed.
         *
         * @param custom The event to convert into a generic event.
	     **************************************************************************************************************/
        template <CustomEventType T>
		Event(const T& custom);
...
template <tr::CustomEventType T>
tr::Event::Event(const T& custom)
{
    CustomEventBase base { custom };
    _impl.type = base.type;
    _impl.user.windowID = base.uint;
    _impl.user.code = base.sint;
    if (base.any1.has_value()) {
        _impl.user.data1 = new std::any { std::move(base.any1) };
    }
    if (base.any2.has_value()) {
        _impl.user.data2 = new std::any { std::move(base.any2) };
    }
}
...
tr::Event::operator TextEditEvent() const noexcept
{
    ...
    event.selected = { event.text.data() + _impl.edit.start, std::size_t(_impl.edit.length) };
    return event;
}
